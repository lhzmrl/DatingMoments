...
import com.yixia.weibo.sdk.VideoProcessEngine$OnLocalMusicLoadListener;
import com.yixia.weibo.sdk.VideoProcessEngine$OnMusicApplyListener;
import com.yixia.weibo.sdk.VideoProcessEngine$OnVideoEncodingListener;
import com.yixia.weibo.sdk.VideoProcessEngine$OnVideoPlayListener;
...
import org.vitamio.vitamiorecorderlibrary.po.POThemeSingle;
import org.vitamio.vitamiorecorderlibrary.view.ThemeGroupLayout;
import org.vitamio.vitamiorecorderlibrary.view.ThemeView;
...
public class MediaPreviewActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		isImportVideo = getIntent().getBooleanExtra(CommonIntentExtra.EXTRA_MEDIA_IMPORT_VIDEO, false);
			...
			public void run() {
				if (resultData != null && resultData.size() > 0) {
					...
					for (VideoMusicModel theme : resultData) {
						...
						musicView.setOnLongClickListener(onlineMusicLongClickListener);
	...
	public void showThemeStore() {
		...
		videoProcessEngine.loadEffectStore(VideoEffectFuncModel.THEME_TYPE_MV, new OnStoreDataLoadListener() {
...
//
//			@Override
//			public void onCompeltedFromCache(VideoEffectStoreModel resultData) {
//				processData(resultData.getVideoEffectModels());
//			}
      ...
      public void onCompeltedFromCache(Object arg0) {
	      // TODO Auto-generated method stub
			...
			@Override
      public void onCompeltedFromNet(Object arg0, boolean arg1, int arg2) {
	      // TODO Auto-generated method stub
				if (arg1) {
					VideoEffectStoreModel resultData = (VideoEffectStoreModel) arg0;
					processData(resultData.getVideoEffectModels());
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				for (VideoEffectModel theme : resultData) {
					...
					Logger.d("videoProcessEngine.theme.previewVideoPath " + theme.previewVideoPath);
					Logger.d("videoProcessEngine.theme.effectPreviewVideoIcon " + theme.effectPreviewVideoIcon);
	...
	private void showLocalMusicList() {
		...
		videoProcessEngine.loadLocalMusic("", new com.yixia.weibo.sdk.VideoProcessEngine$OnLocalMusicLoadListener() {

//			@Override
//			public void onSuccess(ArrayList<VideoMusicModel> mlocalMusicList) {
//				Logger.e("resultData.size() " + mlocalMusicList.size());
...
//			}
			...
			public void onFailed() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						// TODO Auto-generated method stub
						Toast.makeText(MediaPreviewActivity.this, "当前文件夹下未找到mp3音乐文件", Toast.LENGTH_SHORT).show();
						localmusicLayout.setVisibility(View.GONE);
					}
				});
			}
	...
	private void processLocalMusicData(final List<VideoMusicModel> localMusicData) {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				if (localMusicData != null && localMusicData.size() > 0) {
					...
					for (VideoMusicModel theme : localMusicData) {
						TextView localmusicView = new TextView(MediaPreviewActivity.this);
						localmusicView.setTextColor(getResources().getColor(R.color.white));
						localmusicView.setText(theme.effectNameChinese);

						localmusicView.setOnClickListener(mLocalMusicClickListener);
...
//			@Override
//			public void onCompeltedFromCache(List<VideoMusicModel> resultData) {
//
//			}
//
//			@Override
//			public void onCompeltedFromNet(List<VideoMusicModel> resultData, boolean isSuccess, int errorCode) {
...
//			}
      ...
      public void onCompeltedFromCache(Object arg0) {
	      // TODO Auto-generated method stub
			...
			@Override
      public void onCompeltedFromNet(Object resultData, boolean arg1, int arg2) {
	      // TODO Auto-generated method stub
	...
	private void updateMusiclistView() {
		if (musicViews != null && musicViews.size() > 0) {
			...
			TextView t = musicViews.get(videoProcessEngine.getCurrentVideoMusicEffectId());
			if (t != null) {
	...
	private ThemeView addThemeItem(ThemeGroupLayout layout, VideoEffectModel theme, int index) {
		ThemeView themeView = new ThemeView(MediaPreviewActivity.this, theme);
		if (StringUtils.isNotEmpty(theme.effectIconPath)) {
			if (theme.effectIconPath.startsWith("http://") || theme.effectIconPath.startsWith("https://")) {
				themeView.getIcon().setImageResource(R.drawable.empty);
	...
	private OnClickListener mThemeRecommendDownloadClickListener = new OnClickListener() {
		...
		public void onClick(final View v) {
			...
			if (theme == null)
				return;
			if (theme.downloadStatus == com.yixia.weibo.sdk.model.VideoEffectModel$DownloadStatus.DOWNLOADING || theme.downloadStatus == com.yixia.weibo.sdk.model.VideoEffectModel$DownloadStatus.DOWNLOADED) {
				processTheme(theme, v);
				return;
			}
	...
	public class CaptureThumbnailsTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			Boolean result = FFMpegUtils.captureThumbnails(mVideoPath, mCoverPath, String.format("%dx%d", 480, 480));
			// 检查图片大小和视频文件大小
			File videoFile = new File(mVideoPath);
			if (videoFile == null || !videoFile.exists() || videoFile.length() < 50 * 1024) {
				result = false;
			}
			return result;
		}
	...
	private OnLongClickListener onlineMusicLongClickListener = new OnLongClickListener() {
		...
		public boolean onLongClick(View v) {
			...
			if (theme == null)
				return false;
			//未转码
			new AlertDialog.Builder(MediaPreviewActivity.this).setTitle(R.string.hint).setMessage(R.string.delete_onlinemusic_message).setNegativeButton(R.string.record_camera_cancel_dialog_yes, new DialogInterface.OnClickListener() {

				@Override
				public void onClick(DialogInterface dialog, int which) {
					if (theme.effectID == videoProcessEngine.getCurrentVideoMusicEffectId()) {
						Toast.makeText(MediaPreviewActivity.this, "不能删除正在使用的音乐", Toast.LENGTH_SHORT).show();
						return;
					}
					final boolean result = videoProcessEngine.deleteOnlineMusic(theme);
					runOnUiThread(new Runnable() {
						...
						public void run() {
							if (result) {
								musicListLayout.setVisibility(View.GONE);
								Toast.makeText(MediaPreviewActivity.this, "删除成功", Toast.LENGTH_SHORT).show();
							} else {
								Toast.makeText(MediaPreviewActivity.this, "删除失败", Toast.LENGTH_SHORT).show();
	...
	private OnClickListener mLocalMusicClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			//			mPlayStatus.setVisibility(View.GONE);
			//			Logger.d("mCurrentTheme.effectType", ""+mCurrentTheme.effectType);
			//			Logger.d("mCurrentTheme.effectID", ""+mCurrentTheme.effectID);
			//			videoProcessEngine.applyEffect(mCurrentTheme.effectType, mCurrentTheme.effectID, mOnVideoPlayListener);
		}
	};
